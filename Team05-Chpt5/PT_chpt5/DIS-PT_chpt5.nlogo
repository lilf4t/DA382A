...
; setup espresso house
  set espressopatches patches with [ pxcor >= 35 and pxcor <= 54 and pycor >= 20 and pycor <= 33 ]
  ask espressopatches [
    set pcolor brown
    set region "espresso"
  ]
  ask one-of espressopatches [set plabel "espresso"]

  ; setup university
  set universitypatches patches with [ pxcor >= 35 and pxcor <= 54 and pycor >= 1 and pycor <= 15 ]
  ...
  create-citizens num-citizens [
    ...
    set inPrison? false
    set inEspresso? false
    set inUniversity? false
    set jailtime 0
...
to go
  ...
  ask turtles [
    ...
    if (breed = citizens) [
      state_machine ; code as defined in the include-file "citizens.nls"
      ]
    if (breed = cops) [
